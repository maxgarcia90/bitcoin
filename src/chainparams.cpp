...
public:
    CMainParams() {
        ...
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].min_lock_in_time = 0; // No lock in delay

        // Deployment of Taproot (BIPs 340-342)
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].bit = 2;
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nStartTime = 1199145601; // January 1, 2008
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nTimeout = 1230767999; // December 31, 2008
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].min_lock_in_time = 0; // No lock in delay
...
public:
     ...
     */
    void UpdateVersionBitsParameters(Consensus::DeploymentPos d, int64_t nStartTime, int64_t nTimeout, int64_t min_lock_in_time)
    {
        consensus.vDeployments[d].nStartTime = nStartTime;
        consensus.vDeployments[d].nTimeout = nTimeout;
        consensus.vDeployments[d].min_lock_in_time = min_lock_in_time;
    }
...
void CRegTestParams::UpdateActivationParametersFromArgs(const ArgsManager& args)
{
    ...
    for (const std::string& strDeployment : args.GetArgs("-vbparams")) {
        ...
        if (vDeploymentParams.size() < 3 || 4 < vDeploymentParams.size()) {
            throw std::runtime_error("Version bits parameters malformed, expecting deployment:start:end[:min_lock_in]");
        }
        int64_t nStartTime, nTimeout, min_lock_in_time = 0;
        ...
        if (vDeploymentParams.size() > 3 && !ParseInt64(vDeploymentParams[3], &min_lock_in_time)) {
            throw std::runtime_error(strprintf("Invalid min_lock_in_time (%s)", vDeploymentParams[3]));
        }
        bool found = false;
        for (int j=0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
            if (vDeploymentParams[0] == VersionBitsDeploymentInfo[j].name) {
                UpdateVersionBitsParameters(Consensus::DeploymentPos(j), nStartTime, nTimeout, min_lock_in_time);
                found = true;
